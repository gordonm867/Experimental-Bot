...
public class OLDExtension implements Subsystem {
    private Subsystem.State parState;
    public State state;
    ...
    public OLDExtension(State state) {
        this.state = state;
        parState = Subsystem.State.ON;
    }
    ...
    //   • GAMEPAD2.X (BOX DOWN)
    public void update(Gamepad gamepad1, Gamepad gamepad2, TrashHardware robot, RevBulkData data1, RevBulkData data2) {
        if(gamepad2.x && !xchanged && state != State.AUTO && seconded) {
            seconded = false;
            released = false;
            aboutToAutomate = true;
            xchanged = true;
        ...
        if((state == State.AUTO || aboutToAutomate) && !gamepad2.x) {
            released = true;
        }
        if(gamepad2.x && state == State.AUTO && released && !xsecondchanged) {
            state = State.TRANSITIONING;
            xsecondchanged = true;
        }
        if(xsecondchanged && !gamepad2.x) {
            xsecondchanged = false;
        }
        if(!seconded && !gamepad2.x && state != State.AUTO) {
            seconded = true;
        }
        /*
        if(parState == Subsystem.State.ON && (gamepad2.left_stick_y != 0 || state != State.AUTO && state != State.TRANSITIONING)) {
