...
public class SimpleAutoOdometry extends LinearOpMode {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        runToPoint(new Point(0, Math.sqrt(2)));
        telemetry.addData("Point", odometry.getPoint());
        telemetry.update();
        while(opModeIsActive() && !gamepad1.a) {}
        runToPoint(new Point(-1, 1));
        ...
        telemetry.update();
        while(opModeIsActive() && !gamepad1.a) {}
        runToPoint(new Point(-Math.sqrt(2), 0));
        telemetry.addData("Point", odometry.getPoint());
        telemetry.update();
        while(opModeIsActive() && !gamepad1.a) {}
        runToPoint(new Point(-1, -1));
        ...
        telemetry.update();
        while(opModeIsActive() && !gamepad1.a) {}
        double displacement = Math.sqrt(Math.pow(odometry.getX(), 2) + Math.pow(odometry.getY(), 2));
        while(displacement >= 0.1) {
            runToPoint(new Point(0, 0));
            displacement = Math.sqrt(Math.pow(odometry.getX(), 2) + Math.pow(odometry.getY(), 2));
            telemetry.addData("Point", odometry.getPoint());
            telemetry.update();
            while(opModeIsActive() && !gamepad1.a) {}
        }
    ...
    private void runToPoint(double newX, double newY) {
        ...
        double angle;
        try {
            angle = atan(newY - odometry.getY(), newX - odometry.getX());
        }
        ...
        while(opModeIsActive() && !gamepad1.a) {}
        int distance = calculateMove(Math.abs(newX - odometry.getX()), Math.abs(newY - odometry.getY()));
        encoderMovePreciseTimed(distance, 1, Math.abs(distance / 1500.0));
        // while(!gamepad1.a && opModeIsActive()) {}
    }

    private void runToPoint(Point currPoint, Point newPoint) {
        if(!opModeIsActive()) {
            return;
        }
        double angle;
    ...
    private int calculateMove(double xDiff, double yDiff) {
        return((-((int)((Math.sqrt(Math.pow(xDiff, 2) + Math.pow(yDiff, 2))) / Globals.DRIVE_FEET_PER_TICK))));
    }

    private int calculateMove(Point currPoint, Point newPoint) {
        double xDiff = newPoint.getX() - currPoint.getX();
        double yDiff = newPoint.getY() - currPoint.getY();
        return((-((int)((Math.sqrt(Math.pow(xDiff, 2) + Math.pow(yDiff, 2))) / Globals.DRIVE_FEET_PER_TICK))));
    }
