...
@TeleOp(name="INDIV", group="GOF")
@Disabled
public class GOFTeleOpIndiv extends OpMode {
    ...
    public void loop() {
        ...
        double hangDrive = -((gamepad1.dpad_down ? -1 : 0) + (gamepad1.dpad_up ? 1 : 0));
        ...
        if((triggerPressed != 0 || gamepad2.dpad_up || gamepad2.dpad_down || servoMove) || gamepad1.left_trigger != 0) {
            robot.setInPower((gamepad2.dpad_up ? 1 : 0) + triggerPressed - (gamepad2.dpad_down ? 1 : 0) + (servoMove ? 0.4 : 0) - (gamepad1.left_trigger)); // Set intake power based on the gamepad trigger values
        ...
        if(bumperPressed && !(gamepad1.left_bumper || gamepad2.left_bumper)) {
            bumperPressed = false;
        }

        if(gamepad1.a && !gamepad2.start && !aPressed) {
            flipBox(65);
            aPressed = true;
            servoMove = true;
        ...
        if(aPressed && !gamepad1.right_bumper) {
            aPressed = false;
        }
