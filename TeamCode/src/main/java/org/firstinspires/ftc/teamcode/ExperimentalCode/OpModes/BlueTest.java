...
public class BlueTest extends MyOpMode {
    ...
    public void startOp() {
        ...
        if(skystone == 0) {
            ...
            targets.add(new RobotState(-3.7, -2.9, -135, Globals.EXTEND_POS, 0, false, Globals.boxNeutral, 0, false)); // 9
            targets.add(new RobotState(-3.7, -3.2, -135, Globals.EXTEND_POS, -1, true, Globals.boxDown, 3000, true)); // 10
            ...
            targets.add(new RobotState(-3.4, -3, -90, 0, 0, true, Globals.boxNeutral, 0, false)); // 14
            ...
            /* First Block */
            targets.add(new RobotState(-5.25, -3.2, 180, 0, 0, true, Globals.boxNeutral, 0, true)); // 0
            targets.add(new RobotState(-3.5, -3.1, 135, Globals.EXTEND_POS, -1, true, Globals.boxDown, 3000, true)); // 1
            targets.add(new RobotState(-3.5, -3.1, 90, 0, 0, false, Globals.boxNeutral, 0, false)); // 2
            ...
            /* Reposition */
            targets.add(new RobotState(-5.55, 4, 180, 0, 0.6, true, Globals.boxNeutral, 501, true)); // 7
            ...
            targets.add(new RobotState(-3.4, -1.5, 135, Globals.EXTEND_POS, -1, true, Globals.boxDown, 1500, true)); // 12

            /* Place */
            targets.add(new RobotState(-3.4, -1, -90, 0, 0, true, Globals.boxNeutral, 0, false)); // 13
            targets.add(new RobotState(-3.4, 1, -90, 0, 0, true, Globals.boxNeutral, 0, false)); // 14
        ...
        if(((target.isRequired() && target.isExact() && (displacement >= 0.25 || angular >= 3 || (robot.ex != null && Math.abs(expos - target.getExtendPos()) >= 300))) || (!target.isRequired() && !target.isExact() && (displacement >= 0.5)) || (target.isRequired() && !target.isExact() && (displacement >= 0.5 || (robot.ex != null && Math.abs(expos - target.getExtendPos()) >= 300))) || (!target.isRequired() && target.isExact() && (displacement >= 0.25 || angular >= 3)))) {
            ...
            if(target.getDelay() != 0) {
                ...
                while(opModeIsActive() && Math.abs(System.currentTimeMillis() - nowTime) <= target.getDelay()) {
                    if(target.getIntakePower() < 0 && index < 20) {
                        if(data.getMotorCurrentPosition(robot.lift) < 300) {
                            robot.lift.setPower(-0.25);
                        }
                        else {
                    ...
                    if(Math.abs(opTime - System.currentTimeMillis()) >= 28000) {
                        ...
                        while(opModeIsActive() && Math.abs(odometry.getY()) > 0.5) {
                            if(target.getIntakePower() < 0) {
                                if(data.getMotorCurrentPosition(robot.lift) < 300 && index < 20) {
                                    robot.lift.setPower(-0.25);
                                }
                                else {
                                    robot.lift.setPower(0);
