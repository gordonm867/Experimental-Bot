...
public class BlueNewTwoBlock extends MyOpMode {
    ...
    public void startOp() {
        ...
        if(skystone == 0) {
            ...
            targets.add(new RobotState(-2.7, -3.4 + (2f/3f), 135, Globals.EXTEND_POS, -1, true, Globals.boxDown, 3000, true)); // 1
            ...
            targets.add(new RobotState(-3.3, 1.75, -90, 0, 0, false, Globals.boxNeutral , 0, false)); // 9
            targets.add(new RobotState(-3.3, -1, -90, 0, 0, false, Globals.boxNeutral , 0, false)); // 10
            targets.add(new RobotState(-3.3, -2, -135, Globals.EXTEND_POS, 0, false, Globals.boxNeutral, 0, true)); // 11
            targets.add(new RobotState(-2.7, -1.7, -135, Globals.EXTEND_POS, -1, true, Globals.boxDown, 3000, true)); // 12
            ...
            targets.add(new RobotState(-3.3, -2.8, 135, Globals.EXTEND_POS, 0, false, Globals.boxNeutral , 0, true)); // 9
            targets.add(new RobotState(-2.7, -2.8, 135, Globals.EXTEND_POS, -1, true, Globals.boxDown, 3000, true)); // 10
            ...
            targets.add(new RobotState(-2.5, -2.8 - (2f/3f), 90, Globals.EXTEND_POS, -1, true, Globals.boxDown, 3000, true)); // 10
    ...
    public void loopOp() {
        ...
        if(paused == 0 && ((target.isRequired() && target.isExact() && (displacement >= 0.25 || angular >= 3 || (robot.ex != null && Math.abs(expos - target.getExtendPos()) >= 300))) || (!target.isRequired() && !target.isExact() && (displacement >= 0.5)) || (target.isRequired() && !target.isExact() && (displacement >= 0.5 || (robot.ex != null && Math.abs(expos - target.getExtendPos()) >= 300))) || (!target.isRequired() && target.isExact() && (displacement >= 0.25 || angular >= 3)))) {
            ...
            if(iterations > 50 && odometry.getY() > 0 && Math.abs((startPoint.distance(lastPoint) * -Globals.DRIVE_FEET_PER_TICK) / ((lasttime - itertime) / 1000.0)) <= 0.3) { // Move on....
                telemetry.addData("Uh oh", "I'm stuck!");
                telemetry.update();
